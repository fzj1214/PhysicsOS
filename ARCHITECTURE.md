...
next: improve structured semantic validation observability. `plan_backend_preparation_structured` currently retries semantic validation by issuing repeated one-attempt structured calls, so CLI events may show `1/1` for each semantic retry. Extend `call_structured_agent` with an optional semantic validator hook so Pydantic validation and domain validation share one attempt counter and one retry-feedback stream.
next: add contract-aware LLM retry prompts for `validate_numerical_solve_plan` failures. The retry context must include the locked `PhysicsProblemContract`, invalid `NumericalSolvePlanOutput`, Pydantic/semantic validation errors, prior solver-family choice, and a concise instruction to repair only the invalid bindings or fallback decision.
