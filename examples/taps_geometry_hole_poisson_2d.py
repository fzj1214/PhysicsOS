...
def build_problem() -> PhysicsProblem:
    ...
    encodings = generate_geometry_encoding(
        GenerateGeometryEncodingInput(geometry=geometry, encodings=["occupancy_mask"], resolutions=[[40, 40]])
    ).encodings
    geometry.encodings.extend(encodings)
    return PhysicsProblem(
        id="problem:geometry-hole-poisson-2d-demo",
        user_intent={"raw_request": "solve a 2D Poisson equation on a square mask with a circular hole"},
        domain="custom",
        geometry=geometry,
        ...
        provenance=Provenance(created_by="taps_geometry_hole_poisson_2d_example"),
    )
